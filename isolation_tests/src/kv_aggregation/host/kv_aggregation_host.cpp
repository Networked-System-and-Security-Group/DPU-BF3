#define BASE_FAKE_MAC 0x010101010101
...
flexio_func_t kv_aggregation_event_handler;
flexio_func_t kv_aggregation_init;
flexio_func_t kv_aggregation_stop;
flexio_func_t kv_aggregation_query;
};
...
int main(int argc, char **argv) {
    ...
    /* Alloc KV buffer on dpa heap */
    flexio_uintptr_t kv_buf_ptr = 0;
    if (flexio_buf_dev_alloc(fp_ctx->process, sizeof(uint64_t) * NB_ENTRIES,
                             &kv_buf_ptr) != FLEXIO_STATUS_SUCCESS) {
        printf("Failed to alloc kv buffer on dpa heap\n");
        return EXIT_FAILURE;
    }

    uint8_t nb_threads = atoi(argv[2]);
    for (uint8_t thread_id = 0; thread_id < nb_threads; thread_id++) {
        kv_agg_eh_context *kv_agg_eh_ctx =
            new kv_agg_eh_context(fp_ctx->process, thread_id, kv_buf_ptr);
        if (kv_agg_eh_ctx->create_event_handler(kv_aggregation_event_handler) !=
            FLEXIO_STATUS_SUCCESS) {
            printf("Failed to create event handler for thread %d\n", thread_id);
            return EXIT_FAILURE;
        }

        if (kv_agg_eh_ctx->create_queues() != FLEXIO_STATUS_SUCCESS) {
        ...
        if (fp_ctx->steerer->create_rule_rx_mac_match(
                flexio_rq_get_tir(kv_agg_eh_ctx->rq_ctx->flexio_rq_ptr),
        ...
        if (kv_agg_eh_ctx->copy_app_data_to_dpa() != FLEXIO_STATUS_SUCCESS) {
            printf("Failed to copy app data to DPA\n");
            return EXIT_FAILURE;
        }

        uint64_t ret_val = 0;
        if (flexio_process_call(fp_ctx->process, kv_aggregation_init, &ret_val,
                                kv_agg_eh_ctx->app_data_daddr) !=
                FLEXIO_STATUS_SUCCESS ||
            ret_val != 0) {
        ...
        fp_ctx->eh_ctxs->push_back(kv_agg_eh_ctx);
    }
    ...
    uint64_t ret_val = 0;
    if (flexio_process_call(fp_ctx->process, kv_aggregation_stop, &ret_val) !=
            FLEXIO_STATUS_SUCCESS ||
        ret_val != 0) {
    ...
    if (flexio_process_call(fp_ctx->process, kv_aggregation_query,
                            &packets_count,
