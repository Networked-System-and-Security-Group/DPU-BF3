...
static void rx_success_cb1(
    ...
    uint64_t *kv_buf = (uint64_t *)(event_batch_user_data.ptr);
    ...
    for (uint16_t i = 0; i < events_number; i++) {
        ...
        if (result != DOCA_SUCCESS || pkt_buf == NULL) {
            ...
            uint64_t nb_kv_pair = *(uint64_t *)(pkt_buf + 56);
            if (nb_kv_pair == 32) {
                struct kv_pair *kv_pairs = (struct kv_pair *)(pkt_buf + 64);
                for (uint16_t j = 0; j < nb_kv_pair; j++) {
                    struct kv_pair *pair = &kv_pairs[j];
                    kv_buf[pair->key] = pair->value;
                }
            }
        }
    }
...
doca_error_t eth_l2_fwd_execute(struct eth_l2_fwd_cfg *cfg,
    ...
    uint64_t *kv_buf = NULL;
    int res =
        posix_memalign((void *)&kv_buf, 64, sizeof(uint64_t) * NB_ENTRIES);
    if (res != 0 || kv_buf == NULL) {
